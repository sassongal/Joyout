...
class TextProcessor: ObservableObject {
    ...
    // Hebrew-English keyboard mapping (Israeli standard layout)
    private let hebrewToEnglish: [Character: Character] = [
        // Top row
        "ף": "p", "ם": "o", "ן": "i", "ו": "u", "ט": "y", "א": "t", "ר": "r", "ק": "e", "ד": "w", "ג": "q",
        // Middle row  
        "כ": "f", "ע": "g", "י": "h", "ח": "j", "ל": "k", "ך": "l", "פ": ";", ",": "'",
        // Bottom row
        "ז": "a", "ס": "s", "ב": "d", "ה": "f", "נ": "g", "מ": "h", "צ": "j", "ת": "k", "ץ": "l",
        // Special characters
        "/": ".", "=": "\\", "\\": "]", "]": "[", "'": "-", "-": "="
    ]
    ...
    private let englishToHebrew: [Character: Character] = [
        // Top row
        "p": "ף", "o": "ם", "i": "ן", "u": "ו", "y": "ט", "t": "א", "r": "ר", "e": "ק", "w": "ד", "q": "ג",
        // Middle row
        "f": "כ", "g": "ע", "h": "י", "j": "ח", "k": "ל", "l": "ך", ";": "פ", "'": ",",
        // Bottom row
        "a": "ז", "s": "ס", "d": "ב",
        // Special characters
        ".": "/", "\\": "=", "]": "\\", "[": "]", "-": "'", "=": "-"
    ...
    private func fixLayoutSimple(_ text: String) -> String {
        guard !text.trimmingCharacters(in: .whitespaces).isEmpty else { return text }
        
        // Accurate Hebrew-to-English mapping based on Israeli keyboard standard (matches Python)
        ...
        let englishToHebrew = Dictionary(uniqueKeysWithValues: hebrewToEnglish.map { ($0.value, $0.key) })
    ...
    private class LayoutAnalyzer {
        // Complete Hebrew-English QWERTY keyboard mapping (Israeli standard layout)
        private let hebrewToEnglish: [Character: Character] = [
            // Row 1 (top)
            "ף": ";", "ם": "'", "ן": "]", "ו": "[", "ט": "p", "א": "o", "ר": "i", "ק": "u", "ד": "y", "ג": "t", "כ": "r", "ע": "e", "י": "w", "ח": "q",
            // Row 2 (middle)
            "ל": "/", "ך": ".", "פ": ",", "ז": "m", "ס": "n", "ב": "b", "ה": "v", "נ": "c", "מ": "x", "צ": "z", "ת": "a", "ץ": "s",
            // Row 3 (bottom) - corrected
            ",": "d", ".": "f", "/": "g", ";": "h", "'": "j", "]": "k", "[": "l", "=": ";", "\\" : "'", "-": "]"        
        ]
        
        private let englishToHebrew: [Character: Character] = [
            // Row 1 (top)
            "q": "ח", "w": "י", "e": "ע", "r": "כ", "t": "ג", "y": "ד", "u": "ק", "i": "ר", "o": "א", "p": "ט", "[": "ו", "]": "ן",
            // Row 2 (middle)
            "a": "ת", "s": "ץ", "d": ",", "f": ".", "g": "/", "h": ";", "j": "'", "k": "]", "l": "[",
            // Row 3 (bottom)
            "z": "צ", "x": "מ", "c": "נ", "v": "ה", "b": "ב", "n": "ס", "m": "ז", ",": "פ", ".": "ך", "/": "ל",
            // Special characters (avoiding duplicates)
            "'": "ם", ";": "ף"
        ]
        ...
        // Common patterns that strongly suggest layout mistakes
        private let suspiciousEnglishPatterns = [
            "th", "sh", "ch", "ph", "gh", "tion", "ing", "and", "the", "for", "you", "are", "not", "with", "his", "all", "but", "can", "have", "this", "will", "what", "would", "there"
        ]
        
        private let suspiciousHebrewPatterns = [
            "של", "את", "על", "לא", "זה", "או", "אם", "כל", "גם", "הוא", "היא", "אני", "רק", "עם", "יש", "היה", "כי", "אין", "מה", "כמו", "אבל", "רוצה"
        ]
        
        func correctLayout(_ text: String) -> String {
            // Step 1: Analyze the text to determine layout issues
            let analysis = analyzeLayout(text)
            
            switch analysis.detectedIssue {
            case .hebrewTypedInEnglish:
                return convertEnglishToHebrew(text)
            case .englishTypedInHebrew:
                return convertHebrewToEnglish(text)
            case .mixed:
                return handleMixedLayout(text, analysis: analysis)
            case .none:
                return text
            }
        }
        
        private func analyzeLayout(_ text: String) -> LayoutAnalysis {
            var analysis = LayoutAnalysis()
            
            let characters = Array(text.lowercased())
            analysis.totalChars = characters.count
            
            // Count Hebrew and English characters
            for char in characters {
                if hebrewChars.contains(char) {
                    analysis.hebrewChars += 1
                } else if englishChars.contains(char) {
                    analysis.englishChars += 1
                }
            }
            
            // Calculate ratios
            let letterCount = analysis.hebrewChars + analysis.englishChars
            if letterCount > 0 {
                analysis.hebrewRatio = Double(analysis.hebrewChars) / Double(letterCount)
                analysis.englishRatio = Double(analysis.englishChars) / Double(letterCount)
            }
            
            // Analyze patterns for layout mistakes
            analysis.suspiciousEnglishScore = calculatePatternScore(text, patterns: suspiciousEnglishPatterns)
            analysis.suspiciousHebrewScore = calculatePatternScore(text, patterns: suspiciousHebrewPatterns)
            
            // Check for alternating character patterns (strong indicator of layout mistakes)
            analysis.alternatingScore = calculateAlternatingScore(text)
            
            // Determine the most likely issue
            analysis.detectedIssue = determineLayoutIssue(analysis)
            
            return analysis
        }
        
        private func calculatePatternScore(_ text: String, patterns: [String]) -> Double {
            let lowercased = text.lowercased()
            var score = 0.0
            
            for pattern in patterns {
                let occurrences = lowercased.components(separatedBy: pattern).count - 1
                score += Double(occurrences) * Double(pattern.count)
            }
            
            return text.count > 0 ? score / Double(text.count) : 0.0
        }
        
        private func calculateAlternatingScore(_ text: String) -> Double {
            ...
            return Double(alternatingCount) / Double(chars.count - 1)
        }
        
        private func determineLayoutIssue(_ analysis: LayoutAnalysis) -> LayoutIssue {
            print("Layout Analysis:")
            print("  Hebrew ratio: \(analysis.hebrewRatio)")
            print("  English ratio: \(analysis.englishRatio)")
            print("  Suspicious English score: \(analysis.suspiciousEnglishScore)")
            print("  Suspicious Hebrew score: \(analysis.suspiciousHebrewScore)")
            print("  Alternating score: \(analysis.alternatingScore)")
            
            // High alternating score suggests mixed layout mistake
            if analysis.alternatingScore > 0.1 {
                print("  → Detected: Mixed layout (alternating characters)")
                return .mixed
            }
            
            // Predominantly Hebrew characters but suspicious English patterns
            if analysis.hebrewRatio > 0.7 && analysis.suspiciousEnglishScore > 0.02 {
                print("  → Detected: English typed in Hebrew layout")
                return .englishTypedInHebrew
            }
            
            // Predominantly English characters but suspicious Hebrew patterns  
            if analysis.englishRatio > 0.7 && analysis.suspiciousHebrewScore > 0.02 {
                print("  → Detected: Hebrew typed in English layout")
                return .hebrewTypedInEnglish
            }
            
            // Lower threshold for mixed content
            if analysis.hebrewRatio > 0.1 && analysis.englishRatio > 0.1 {
                if analysis.suspiciousEnglishScore > 0.01 || analysis.suspiciousHebrewScore > 0.01 {
                    print("  → Detected: Mixed content with layout issues")
                    return .mixed
                }
            }
            
            // Simple heuristic: if we have mostly one language but some characters from the other
            if analysis.hebrewRatio > 0.5 && analysis.englishRatio > 0.1 {
                print("  → Detected: Likely English typed in Hebrew layout (ratio-based)")
                return .englishTypedInHebrew
            }
            
            if analysis.englishRatio > 0.5 && analysis.hebrewRatio > 0.1 {
                print("  → Detected: Likely Hebrew typed in English layout (ratio-based)")
                return .hebrewTypedInEnglish
            }
            
            print("  → No layout issue detected")
            return .none
        }
        
        private func convertHebrewToEnglish(_ text: String) -> String {
            return String(text.compactMap { char in
                if let englishChar = hebrewToEnglish[char] {
                    return englishChar
                }
                return char
            })
        ...
        private func convertEnglishToHebrew(_ text: String) -> String {
            return String(text.compactMap { char in
                if let hebrewChar = englishToHebrew[Character(char.lowercased())] {
                    return char.isUppercase ? hebrewChar : hebrewChar
                }
                return char
            })
        }
        
        private func handleMixedLayout(_ text: String, analysis: LayoutAnalysis) -> String {
            // For mixed layout, try to identify segments and convert appropriately
            var result = ""
            var currentSegment = ""
            var segmentIsHebrew = false
            
            for char in text {
                let charIsHebrew = hebrewChars.contains(Character(char.lowercased()))
                let charIsEnglish = englishChars.contains(Character(char.lowercased()))
                
                if charIsHebrew || charIsEnglish {
                    if currentSegment.isEmpty {
                        // Start new segment
                        currentSegment = String(char)
                        segmentIsHebrew = charIsHebrew
                    } else if (segmentIsHebrew && charIsHebrew) || (!segmentIsHebrew && charIsEnglish) {
                        // Continue current segment
                        currentSegment += String(char)
                    } else {
                        // Switch detected - process current segment
                        result += processSegment(currentSegment, isHebrew: segmentIsHebrew)
                        currentSegment = String(char)
                        segmentIsHebrew = charIsHebrew
                    }
                } else {
                    // Non-letter character - process current segment and add the character
                    if !currentSegment.isEmpty {
                        result += processSegment(currentSegment, isHebrew: segmentIsHebrew)
                        currentSegment = ""
                    }
        ...
        private func processSegment(_ segment: String, isHebrew: Bool) -> String {
            // Analyze if this segment looks like a layout mistake
            let patternScore = isHebrew ? 
                calculatePatternScore(segment, patterns: suspiciousEnglishPatterns) :
                calculatePatternScore(segment, patterns: suspiciousHebrewPatterns)
            
            // If the pattern score suggests a layout mistake, convert it
            if patternScore > 0.1 {
                return isHebrew ? convertHebrewToEnglish(segment) : convertEnglishToHebrew(segment)
            }
    ...
    private struct LayoutAnalysis {
        ...
        var englishChars = 0
        var hebrewRatio = 0.0
        var englishRatio = 0.0
        var suspiciousEnglishScore = 0.0
        var suspiciousHebrewScore = 0.0
        var alternatingScore = 0.0
        var detectedIssue: LayoutIssue = .none
    }
    
    private enum LayoutIssue {
        case none
        case hebrewTypedInEnglish
        case englishTypedInHebrew
        case mixed
